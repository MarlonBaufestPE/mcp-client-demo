...
function formatBookDetails(bookDetail) {
    return [
...
server.tool("get-book-by-isbn", "Get book details from ISBN", {
    ...
    const book = booksDetails.books.find((b) => b.ISBN === isbn);
    if (!book) {
...
server.tool("get-book-by-title", "Get book basic information from title", {
...
server.tool("get-books-by-titles", "Get books basic information from a list of titles", {
    titles: z.array(z.string()).describe("The titles of the books to retrieve"),
}, async ({ titles }) => {
    // retrieve book basic information from the src/data/books.json file
    const foundBooks = books.filter((b) => titles.includes(b.title));
    if (foundBooks.length === 0) {
        throw new Error(`No books found for titles: ${titles.join(", ")}`);
    }
    return {
        content: foundBooks.map((book) => ({
            type: "text",
            text: formatBook(book),
        })),
    };
});
server.tool("get-books-by-isbn-list", "Get books details from a list of ISBNs", {
    isbnList: z.array(z.string().length(10)).describe("The list of ISBNs of the books to retrieve"),
}, async ({ isbnList }) => {
    // retrieve book basic information from the src/data/books.json file
    const foundBooks = booksDetails.books.filter((b) => isbnList.includes(b.ISBN));
    if (foundBooks.length === 0) {
